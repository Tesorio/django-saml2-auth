...
4. In `settings.py`, add the SAML2 related configuration:
    ...
    SAML2_AUTH = {
        ...
        'TRIGGER': {
            ...
            # Receives the identity as the IdP asserted it, keyed by SAML attribute name
        ...
        'USER_LOOKUP_FIELD': 'email',  # Model field the asserted identity is matched against. Defaults to the model's USERNAME_FIELD
        'ERROR_REDIRECTS': {  # Answer these recoverable failures with a redirect instead of the error page. Each value is a URL path or a URL pattern name; an unset key keeps the error page
            'NO_SAML_RESPONSE': 'django_saml2_auth:denied',  # The POST to acs carried no SAMLResponse
            'NO_METADATA': 'sso_login',  # GET_METADATA_FROM_REQUEST could not identify a tenant
            'USER_NOT_FOUND': '/login/?no_user=true',  # No such user, and CREATE_USER is False
            'INACTIVE_USER': 'django_saml2_auth:denied',  # The user exists but is not active
        },
...
| **USER\_LOOKUP\_FIELD**                     | Model field the asserted identity is looked up by. An IdP usually asserts an email while the local username is unrelated, in which case set this to `email`. Matched case-insensitively unless `LOGIN_CASE_SENSITIVE` is set                                                                                                                                                                                                                 | `str`            | `None` (uses the model's `USERNAME_FIELD`)                                                                                               | `email`                                                  |
| **ERROR\_REDIRECTS**                       | Recoverable login failures to answer with a redirect instead of the error page, keyed by `NO_SAML_RESPONSE`, `NO_METADATA`, `USER_NOT_FOUND` and `INACTIVE_USER`. Each value is a URL path, an absolute URL, or a URL pattern name to reverse. A key left unset keeps the error page. An SSO-only user has no password to fall back on, so an error page strands them where a redirect puts them back on a route they can act on | `dict`           | `{}`                                                                                                                                     | `{'INACTIVE_USER': 'django_saml2_auth:denied'}`           |
